...
__global__ void gemm_Kernel(const int B, const int C, const int H, const int W, const int M, const int K, float *X_unrolled, float *y)
{
    ...
    int h_out = col / W_out, w_out = col % W_out;
    int H_unroll = C * K * K, W_unroll = H_out * W_out;
    float result = 0;
    
    #pragma unroll
    for(int ph = 0;ph < ceil(H_unroll / TILE_SIZE);ph++)
    {
        int c = (ph * TILE_SIZE_ + ty) / C;
        int s = (ph * TILE_SIZE_ + ty) % C;
        int h = s / K, w = s % K;

        
        if(ph * TILE_SIZE + ty < H_unroll && col < W_unroll)
            input[ty * TILE_SIZE_ + tx] = X_unrolled[B * C * H * W + c * H * W + (h_out + h) * W + (w_out + w)];
        else
    ...
    if(row < M && col < W_unroll)
        y[B * W_unroll * M + row * W_unroll + col] = result;
...
void forward<gpu, float>(mshadow::Tensor<gpu, 4, float> &y, const mshadow::Tensor<gpu, 4, float> &x, const mshadow::Tensor<gpu, 4, float> &w)
{
    ...
    //{
        ...
        for(int j = 0;j < B;j++)
        {
            ...
            gemm_Kernel<<<dimGrid, dimBlock, 0>>>(j, C, H, W, M, K, x.dptr_, y.dptr_);
            // MSHADOW_CUDA_CALL(cudaDeviceSynchronize()); 
        }
    //}
